...
class ProviderController extends Controller
{
    ...
    public function dashboard(Request $request, Transaction $transaction, SubscribedService $subscribedService, Serviceman $serviceman): JsonResponse
    {
        ...
        if (in_array('recent_bookings', $request['sections'])) {
            $subscribed_sub_categories = $this->subscribedService
                ...
                ->where(['is_subscribed' => 0])->pluck('sub_category_id')->toArray();
    ...
    public function config(Request $request): JsonResponse
    {
        ...
        return response()->json(response_formatter(DEFAULT_200, [
            ...
            'app_url_android' => 'https://play.google.com/store/apps/details?id=com.download.repair_provider',
            'app_url_ios' => 'https://apps.apple.com/us/app/repair-provider/id6468879493',
    ...
    public function get_company_details(Request $request): JsonResponse
    {
        $providers = $this->provider->with(['owner', 'zone'])->where(['is_approved' => 1])->withCount(['subscribed_services', 'bookings'])->first();
