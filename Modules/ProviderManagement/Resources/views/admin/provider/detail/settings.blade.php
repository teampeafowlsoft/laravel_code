        ...
        <div class="container-fluid">
            ...
            <div class="mb-3">
                <ul class="nav nav--tabs nav--tabs__style2">
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{$web_page=='order'?'active':''}}"
                           href="{{url()->current()}}?web_page=order">{{translate('Orders')}}</a>
                    </li>
                    <li class="nav-item">
                ...
                <div class="card-body p-30">
                    <form action="{{route('admin.provider.commission_update', [$provider->id])}}" method="post">
                        ...
                        <div class="form-floating {{$provider->commission_status == 0 ? 'd-none' : ''}}" id="percentage">
                            <input type="number" min="0" max="100" step="any" class="form-control" placeholder="{{translate('Service_Percentage')}}"
                            ...
                            <label>{{translate('Service_Percentage')}}</label>
                            <small class="text-danger d-flex mt-1">*{{translate('whatever_commission_write_here_admin_will_get_this_commission')}}</small>
                        </div>
                        <br>
                        <br>
                        <div class="form-floating {{$provider->commission_status == 0 ? 'd-none' : ''}}" id="product_percentage">
                            <input type="number" min="0" max="100" step="any" class="form-control" placeholder="{{translate('Product_Percentage')}}"
                                   id="product_percentage__input" name="product_commission_value" value="{{$provider->commission_percentage_product}}"
                                {{$provider->commission_status == 1 ? 'required' : ''}}>
                            <label>{{translate('Product_Percentage')}}</label>
                        </div>
    ...
    <script>
        $('#default_commission').click(function() {
            if($('#default_commission').is(':checked')) {
                ...
                $('#product_percentage').addClass( 'd-none');
                $('#product_percentage__input').removeAttr('required');
            }
        });
        ...
        $('#custom_commission').click(function() {
            if($('#custom_commission').is(':checked')) {
                ...
                $('#product_percentage').removeClass( 'd-none');
                $('#product_percentage__input').prop('required',true);
            }
        });
    </script>
@endpush
