            ...
            <main>
                ...
                <table border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        ...
                        <th class="text-right text-uppercase">{{translate('final_cost')}}</th>
                    ...
                    @foreach($booking->detail as $index=>$item)
                        <tr>
                            ...
                            <td class="text-left">
                                ...
                                {{$item->variation != null ? $item->variation->attribute_value . ' ' . $item->variation->attribute_name : ''}}
                            </td>
                            <td class="unit">{{with_currency_symbol($item->service_cost)}}</td>
                            <td class="qty">{{$item->quantity}}</td>
                            <td class="discount_amount text-right">{{$item->discount_amount}}</td>
                    ...
                    <tr>
                        <td colspan="4"></td>
                        <td class="text-uppercase">{{translate('subtotal')}}</td>
                        <td>{{with_currency_symbol($sub_total)}}</td>
                    </tr>
{{--                    <tr>--}}
{{--                        <td colspan="4"></td>--}}
{{--                        <td class="text-uppercase">{{translate('Discount')}} </td>--}}
{{--                        <td>{{with_currency_symbol($booking->total_discount_amount)}}</td>--}}
{{--                    </tr>--}}
                    <tr>
                        <td colspan="4"></td>
                        <td class="text-uppercase">{{translate('Campaign_Discount')}} </td>
                        <td>(-) {{with_currency_symbol($booking->total_campaign_discount_amount)}}</td>
                    </tr>
                    <tr>
                        <td colspan="4"></td>
                        <td class="text-uppercase">{{translate('Coupon_Discount')}} </td>
                        <td>(-) {{with_currency_symbol($booking->total_coupon_discount_amount)}}</td>
                    </tr>
                    <tr>
                        <td colspan="4"></td>
                        <td class="text-uppercase">{{translate('delivery_charge')}}</td>
                    ...
                    <tr>
                        <td colspan="4"></td>
                        <td class="text-uppercase">{{translate('grand_total')}}</td>
                        <td>{{with_currency_symbol(($sub_total + $booking->total_tax_amount + $booking->total_delivery_charge + $booking->total_shipping_charge) - ($booking->total_coupon_discount_amount + $booking->total_campaign_discount_amount))}}</td>
                    </tr>
                    </tfoot>
                </table>
