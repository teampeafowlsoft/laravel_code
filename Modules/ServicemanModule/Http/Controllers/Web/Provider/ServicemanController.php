...
class ServicemanController extends Controller
{
    ...
    {
        $checkEmail = $this->employee->where('user_type','provider-serviceman')->where('email',$request->email)->count();
        $checkPhone = $this->employee->where('user_type','provider-serviceman')->where('phone',$request->phone)->count();

        $request->validate([
            'first_name' => 'required',
            'last_name' => 'required',
            'phone' => ($checkPhone > 0) ? 'required|regex:/^([0-9\s\-\+\(\)]*)$/|min:10|unique:users' : 'required',
            'email' => ($checkEmail > 0) ? 'required|email|unique:users,email' : 'required|email',
            ...
            'identity_image.*' => 'image|mimes:jpeg,jpg,png,gif',
        ]);
        ...
        DB::transaction(function () use ($request, $identity_images) {
            ...
            $employee->country_code = $request->sign.$request->country_code;
    ...
    public function show(Request $request, string $id)
    {
        ...
        $serviceman = $this->serviceman::with(['user.addresses'])
            ->withCount('bookings as total_assigned_bookings')
            ->withCount(['bookings as total_ongoing_bookings' => function ($query) {
                $query->where('booking_status', 'ongoing');
            }])
            ->withCount(['bookings as total_completed_bookings' => function ($query) {
                $query->where('booking_status', 'completed');
            }])
            ->withCount(['bookings as total_canceled_bookings' => function ($query) {
        ...
        return view('servicemanmodule::Provider.Serviceman.details', compact('serviceman', 'chart_data', 'date_range'));
    }
    ...
    public function update(Request $request, string $id): RedirectResponse
    {
        ...
        $request->validate([
            'first_name' => 'required',
            'last_name' => 'required',
            'phone' => ($request->mobile !== $employee->mobile)?'required|unique:users,phone,' . $employee->id : '',
            'email' => 'required|email|unique:users,email,' . $employee->id,
            'password' => '',
            'confirm_password' => !is_null($request->password) ? 'required|min:8|same:password' : '',
            'profile_image' => 'image|mimes:jpeg,jpg,png,gif',
