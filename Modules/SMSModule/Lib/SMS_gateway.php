...
class SMS_gateway
{
    public static function send($receiver, $otp): string
    {
        ...
        $config = self::get_settings('mpp');
        if (isset($config) && $config['status'] == 1) {
            return self::mpp_sms($receiver, $otp);
        }
    ...
    public static function mpp_sms($receiver, $otp): string
    {
        $config = self::get_settings('mpp');
        $response = 'error';
        if (isset($config) && $config['status'] == 1) {
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://api.mpp-sms.com/api/send.aspx?apikey=" . $config['auth_key'] . "&language=1&sender=" . $config['template_id'] . "&mobile=".$receiver."&message=Your%20verification%20code%20is%20".$otp,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
            ...
            $response = curl_exec($curl);
            $response_test = explode(',',$response);
            if (count($response_test)){
                if ($response[0] != 'error') {
                    $response = 'success';
                } else {
                    $response = 'error';
