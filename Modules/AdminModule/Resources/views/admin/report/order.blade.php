            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-wrap mb-3">
                        <h2 class="page-title">{{translate('Order_Reports')}}</h2>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{route('admin.report.order')}}" method="POST">
                                @csrf
                                <div class="row">
                                ...
                                <div class="card p-30 flex-grow-1">
                                    <div class="d-flex gap-4 flex-wrap">
                                        ...
                                        <div class="text-center">
                                            ...
                                            <span class="fz-12">{{translate('total_orders')}}</span>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-wrap justify-content-between gap-2 mt-30">
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="fw-semibold text-danger">{{$bookings_count['cancelled']}}</span>
                                            <span class="opacity-50">{{translate('Canceled')}}</span>
                                        </div>
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="fw-semibold text-success">{{$bookings_count['processed']}}</span>
                                            <span class="opacity-50">{{translate('order_processed')}}</span>
                                        </div>
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="c1 fw-semibold">{{$bookings_count['shipped']}}</span>
                                            <span class="opacity-50">{{translate('order_shipped')}}</span>
                                        </div>
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="fw-semibold text-success">{{$bookings_count['delivered']}}</span>
                                            <span class="opacity-50">{{translate('order_delivered')}}</span>
                                        </div>
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="fw-semibold text-success">{{$bookings_count['received']}}</span>
                                            <span class="opacity-50">{{translate('order_received')}}</span>
                                        </div>
                                    </div>
                                </div>
                                <!-- End Card -->
                        ...
                        <div class="col-xl-9">
                            <div class="card">
                                <div class="card-body ps-0">
                                    <h4 class="ps-20">{{translate('order_statistics')}}</h4>
                                ...
                                <div class="d-flex flex-wrap align-items-center gap-3">
                                    <div>
                                        <select class="js-select booking-status__select" name="booking_status" id="booking-status">
                                            <option value="" selected disabled>{{translate('Order_status')}}</option>
                                            <option value="all">{{translate('All')}}</option>
                                            @foreach(ORDER_STATUSES as $booking_status)
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                                            <li><a class="dropdown-item" href="{{route('admin.report.order.download').'?'.http_build_query(request()->all())}}">{{translate('Excel')}}</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table class="table align-middle">
                                    ...
                                    @forelse($filtered_bookings as $key=>$booking)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('admin.orderpool.details', [$booking->id,'web_page'=>'details'])}}">
                                                    {{$booking['readable_id']}}
                                                </a>
                                            </td>
                                            <td>
    ...
    <script>
        $(document).ready(function () {
            $('#booking-status').on('change', function() {
                location.href = "{{route('admin.report.order')}}" + "?booking_status=" + this.value;
            });
        });
    </script>
