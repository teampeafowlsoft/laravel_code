...
class OverviewReportController extends Controller
{
    ...
    public function get_business_overview_report(Request $request)
    {
        ...
        $categories = $this->categories->ofType('main')->select('id', 'name')->where('lang_id',1)->get();
        $sub_categories = $this->categories->ofType('sub')->select('id', 'name')->where('lang_id',1)->get();
    ...
    public function get_business_overview_report_download(Request $request)
    {
        ...
        $amounts = $this->booking_details_amount
            ...
            ->when($deterministic == 'month', function ($query) use ($deterministic) {
                $query->select(
                    DB::raw('sum(service_unit_cost) as service_unit_cost'),
                    DB::raw('sum(discount_by_admin) as discount_by_admin'),
                    DB::raw('sum(discount_by_provider) as discount_by_provider'),
            ...
            ->when($deterministic == 'year', function ($query) use ($deterministic) {
                $query->select(
                    DB::raw('sum(service_unit_cost) as service_unit_cost'),
                    DB::raw('sum(discount_by_admin) as discount_by_admin'),
                    DB::raw('sum(discount_by_provider) as discount_by_provider'),
            ...
            ->when(($deterministic == 'day' || $deterministic == 'week'), function ($query) use ($deterministic) {
                $query->select(
                    DB::raw('sum(service_unit_cost) as service_unit_cost'),
                    DB::raw('sum(discount_by_admin) as discount_by_admin'),
                    DB::raw('sum(discount_by_provider) as discount_by_provider'),
            ...
            ->groupby($deterministic=='week'?'day':$deterministic)
            ->get();

        return (new FastExcel($amounts))->download(time().'-business-overview-report.xlsx');
    }
}
