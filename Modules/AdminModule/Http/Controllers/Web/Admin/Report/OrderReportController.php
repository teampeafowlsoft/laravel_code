...
class OrderReportController extends Controller
{
    ...
    protected Productcategory $productcategories;
    protected ProductCartBooking $productcartbooking;
    ...
    public function __construct(Zone $zone, Provider $provider, Productcategory $productcategories, Service $service, ProductCartBooking $productcartbooking, Account $account, User $user, Transaction $transaction, ProductCartBookingDetailsAmount $product_cart_booking_details_amount)
    {
        $this->zone = $zone;
        ...
        $this->product_cart_booking_details_amount = $product_cart_booking_details_amount;
    }
    ...
    public function get_order_report(Request $request)
    {
        Validator::make($request->all(), [
            ...
            'booking_status' => 'in:' . implode(',', array_column(ORDER_STATUSES, 'key')) . ',all',
        ]);


        //Dropdown data
        $zones = $this->zone->ofStatus(1)->select('id', 'name')->where('lang_id',1)->get();
        $providers = $this->provider->ofApproval(1)->select('id', 'company_name', 'company_phone')->get();
        $categories = $this->productcategories->ofType('main')->select('id', 'name')->where('lang_id',1)->get();
        $sub_categories = $this->productcategories->ofType('sub')->select('id', 'name')->where('lang_id',1)->get();
        ...
        //** Table Data **
        $filtered_bookings = self::filter_query($this->productcartbooking, $request)
        ...
        //** Card Data **
        $bookings_for_amount = self::filter_query($this->productcartbooking, $request)
            ->with(['customer', 'provider.owner'])
            ->whereIn('booking_status', ['processed', 'shipped', 'delivered', 'received', 'cancelled'])
            ->get();

        $bookings_count = [];
        $bookings_count['total_bookings'] = $bookings_for_amount->count();
        $bookings_count['processed'] = $bookings_for_amount->where('booking_status', 'processed')->count();
        $bookings_count['shipped'] = $bookings_for_amount->where('booking_status', 'shipped')->count();
        $bookings_count['delivered'] = $bookings_for_amount->where('booking_status', 'delivered')->count();
        $bookings_count['received'] = $bookings_for_amount->where('booking_status', 'received')->count();
        $bookings_count['cancelled'] = $bookings_for_amount->where('booking_status', 'cancelled')->count();

        $booking_amount = [];
        $booking_amount['total_booking_amount'] = $bookings_for_amount->sum('total_booking_amount');
        $booking_amount['total_paid_booking_amount'] = $bookings_for_amount->where('booking_status', 'received')->where('is_paid', 1)->sum('total_booking_amount');
        ...
        $amounts = $this->product_cart_booking_details_amount
            ->whereHas('booking', function ($query) use ($request) {
                self::filter_query($query, $request)->whereIn('booking_status', ['processed', 'shipped', 'delivered', 'received', 'cancelled']);
            })
        ...
        $bookings = self::filter_query($this->productcartbooking, $request)
            ->whereIn('booking_status', ['processed', 'shipped', 'delivered', 'received', 'cancelled'])
            ->when(isset($group_by_deterministic), function ($query) use ($group_by_deterministic) {
                $query->select(
        ...
        return view('adminmodule::admin.report.order', compact('zones', 'providers', 'categories', 'sub_categories', 'search', 'filtered_bookings', 'bookings_count', 'booking_amount', 'chart_data', 'query_params'));
    }

    public function get_order_report_download(Request $request): string|StreamedResponse
    {
        Validator::make($request->all(), [
            'zone_ids' => 'array',
            'zone_ids.*' => 'uuid',
        ...
        return (new FastExcel($filtered_bookings))->download(time().'-order-report.xlsx', function ($booking) {
            return [
