...
//                    ->orWhere('zone_id', $request->user()->provider->zone_id);
//            })
...
//            })
////            ->where('provider_id', $request->user()->provider->id)
//            ->where('provider_selected_ids', 'LIKE', '%'.$request->user()->provider->id.'%')
//            ->latest()->paginate(pagination_limit())->appends($query_param);
        ...
        $bookings = $this->booking->with(['customer'])
            ...
            ->when($booking_status != 'all', function ($query) use ($booking_status, $request) {
...
//
    ...
    public function details($id, Request $request)
    {
        ...
        elseif ($request->web_page == 'images') {
            ...
            return view('bookingmodule::provider.booking.images', compact('booking', 'web_page'));
        }
    }
