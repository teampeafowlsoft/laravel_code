...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, BookingStatusHistory $booking_status_history,BookingDetail $bookingdetail)
    {
    ...
    public function payment_status_update(Request $request, string $booking_id): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            $booking_details = $this->bookingdetail->where('booking_id',$booking_id)->where('is_paid','0')->get();

            if (!empty($booking_details)){
                foreach ($booking_details as $booking_detail){
                    $booking_detail->is_paid = '1';
                    $booking_detail->save();
                }
            }
