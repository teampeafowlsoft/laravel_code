...
class CustomerController extends Controller
{
    ...
    public function __construct(Booking $booking, ProductCartBooking $productcartbooking, User $user, Review $review, UserAddress $address)
    {
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $request->validate([
            ...
            'phone' => ($checkPhone > 0) ? 'required|regex:/^([0-9\s\-\+\(\)]*)$/|min:10|unique:users' : 'required|regex:/^([0-9\s\-\+\(\)]*)$/|min:10',
...
//            'profile_image' => 'image|mimes:jpeg,jpg,png,gif|max:10000',
    ...
    public function update(Request $request, string $id): Redirector|RedirectResponse|Application
    {
        ...
        $request->validate([
            'first_name' => 'required',
            'last_name' => 'required',
            'email' => ($checkEmail > 0) ? 'required|email|unique:users,email,' . $customer->id : 'required|email',
            'phone' => ($checkPhone > 0) ? ($request->mobile !== $customer->mobile)?'required|regex:/^([0-9\s\-\+\(\)]*)$/|min:10|unique:users,phone,' . $customer->id : '' : 'required|regex:/^([0-9\s\-\+\(\)]*)$/|min:10',
            'password' => '',
...
//            'profile_image' => 'image|mimes:jpeg,jpg,png,gif|max:10000',
        ...
        $customer->country_code = $request->sign.$request->country_code;
...
//        exit();
    ...
    {
        $request->validate([
            'web_page' => 'in:overview,bookings,orders,reviews',
        ]);
        ...
        //orders
        elseif ($request->web_page == 'orders') {

            $search = $request->has('search') ? $request['search'] : '';
            $query_param = ['web_page' => $web_page, 'search' => $search];

            $bookings = $this->productcartbooking->with(['provider.owner'])
                ->where('customer_id', $id)
                ->where(function ($query) use ($request) {
                    $keys = explode(' ', $request['search']);
                    foreach ($keys as $key) {
            ...
            return view('customermodule::admin.detail.orders', compact( 'bookings', 'web_page'));
