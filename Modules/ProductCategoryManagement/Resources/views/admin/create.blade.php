...
@section('title',translate('product_category_setup'))
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-wrap mb-3">
                        <h2 class="page-title">{{translate('product_category_setup')}}</h2>
                    </div>
                                                        ...
                                                        <div class="form-floating mb-30">
                                                            <select class="select-zone theme-input-style w-100"
                                                                ...
                                                                @foreach($arabic_zones as $arabic_zone)
                                                                    <option
                                                                        value="{{$arabic_zone['id']}}">{{$arabic_zone->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                            ...
                                            @foreach($categories as $key=>$category)
                                                ...
                                                <tr>
                                                    ...
                                                    <td><img
                                                            width="70" height="70"
                                                            onerror="this.src='{{asset('public/assets/admin-module')}}/img/media/upload-file.png'"
                                                            src="{{asset('storage/app/public/productcategory')}}/{{explode(',',$category->image)[0]}}"
                                                            alt=""></td>
                                                    ...
                                                    <td>
                                                        <label class="switcher" data-bs-toggle="modal"
                                                            ...
                                                            <input class="switcher_input"
                                                                   onclick="route_alert('{{route('admin.productcategory.status-update',[$category->group_id])}}','{{translate('want_to_update_status')}}')"
                                                    ...
                                                    <td>
                                                        <div class="table-actions">
                                                            <a href="{{route('admin.productcategory.edit',[$category->id,$category->group_id])}}"
                                                            ...
                                                            <form
                                                                action="{{route('admin.productcategory.delete',[$category->group_id])}}"
...
{{--            });--}}
{{--            $.ajax({--}}
{{--                url: "{{route('admin.productcategory.lang-translate')}}",--}}
...
{{--    </script>--}}
    ...
    <script>
        $('#eng_zone_id').change(function () {
           var eng_zone_id = $('#eng_zone_id').val();
           // alert(eng_zone_id);
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            $.ajax({
                url: "{{route('admin.productcategory.zone-select')}}",
                data: {eng_zone_id: eng_zone_id},
                method: "GET",
                dataType: 'json',
                success: function (response) {
                    console.log(response);
