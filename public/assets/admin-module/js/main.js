...
(function ($) {
  ...
  ====================================*/
  $(document).ready(function() {
    $('.table td input[type=checkbox]').on('change', function() {
      let checkedLength = $('.table td input[type=checkbox]:checked').length;
        if(this.checked) {
          $(this).parents('tr').addClass('bg-light');
        } else {
          $(this).parents('tr').removeClass('bg-light')
        }
        $(this).parents('.table-responsive').siblings('.multiple-select-actions').find('.checked-count').html(checkedLength);

        if(checkedLength >= 2) {
          $(this).parents('.table-responsive').siblings('.multiple-select-actions').addClass('active');
          $(this).parents('table').find('thead').hide();
        } else {
          $(this).parents('.table-responsive').siblings('.multiple-select-actions').removeClass('active');
          $(this).parents('table').find('thead').show();
        }
    });
    $('.multi-checker').on('change', function() {
      let tableResponsive = $(this).parents('.multiple-select-actions').siblings('.table-responsive');
      if(this.checked) {
          tableResponsive.find('input[type=checkbox]').prop("checked", true);
          tableResponsive.find('tr').addClass("bg-light");
          tableResponsive.find('thead').hide();
        } else {
          tableResponsive.find('input[type=checkbox]').prop("checked", false);
          $(this).parents('.multiple-select-actions').removeClass('active');
          tableResponsive.find('tr').removeClass('bg-light');
          tableResponsive.find('thead').show();
        }
      $(this).parents('.multiple-select-actions').find('.checked-count').html($('.table td input[type=checkbox]:checked').length);
    });
});
